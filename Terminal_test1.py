...
if __name__ == "__main__":
    ...
    # To make the game case-insensitive for item and object matching, update all direct string comparisons
    # (e.g., in use_item, examine, take, drop, etc.) to use a helper that finds the canonical name
    # regardless of case. This will not damage any aspect if you always use the canonical name for state changes.

    # Add this helper to final_destination_terminal.py:
    def find_canonical_name(name, collection):
        """Return the canonical name from collection matching name case-insensitively, or None."""
        name = name.lower()
    ...
    # Example usage in use_item (replace direct comparisons):
    def use_item(command):
        # ... (other code)
        if len(command) < 4 or command[2].lower() != "on":
            print("Use which item on what?")
            return False

        item_to_use_input = command[1]
        target_object_input = " ".join(command[3:])

        # Use case-insensitive matching for inventory and room objects
        item_to_use = find_canonical_name(item_to_use_input, player['inventory'])
        room_data = rooms[player['location']]
        target_object = find_canonical_name(target_object_input, room_data.get('objects', []))

        if not item_to_use:
            print(f"You don't have a {item_to_use_input}.")
            return False
        if not target_object:
            print(f"You don't see a {target_object_input} here.")
            return False

        # Now use item_to_use and target_object for all logic below
        # Example:
    ...
    # Do the same in examine, take, drop, etc.:
    # Replace any direct string comparison with find_canonical_name and always use the canonical name.

    # This change will make all item/object interactions case-insensitive and will not break game logic,
    # as long as you always use the canonical name for state changes and output.